/* HITLERSTORE app.js v8 - Telegram */

const CFG = { wa:"201278443292", rate:50, cur:"EGP" };
const TG_TOKEN = "8652212926:AAEkceLX2JehviVTK9CDYCKiAzPsj3Arl7M";
const TG_CHAT  = "762758025";
...
async function sendToTelegram(order){
  const waLink = `https://wa.me/${order.phone}`;
  const tgLink = `https://t.me/${order.phone}`;
  const msg = 
`🛒 *طلب جديد!*
...
👾 *اسم اللعبة:* ${order.gameName}
📦 *الباقة:* ${order.package.label}
💰 *السعر:* ${order.package.price.toLocaleString()} جنيه
🆔 *رقم الطلب:* #${order.orderId}`;

  const keyboard = {
    inline_keyboard: [[
      {text:"📞 واتساب العميل", url: waLink},
      {text:"✅ تم الشحن", callback_data:"done_"+order.orderId}
    ]]
  };
...
window.submitOrder=async function(){
  ...
  try{
    ...
    localStorage.setItem("hs_last_order", orderId);
    closeMo();
