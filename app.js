...
// LOADER
// ====================================
// إخفاء الـ loader — مش بيستنى تحميل الصور
function hideLoader() {
  const loader = document.getElementById("loader");
  if (loader) loader.classList.add("hidden");
}

// يختفي بعد 2.2 ثانية على طول بغض النظر عن أي حاجة
setTimeout(hideLoader, 2200);

// لو DOM خلص قبل كده يختفي بعد 1.8 ثانية
  ...
  grid.innerHTML = DATA.uc_packages.map((pkg, i) => {
    ...
    return `
      <div class="pkg-card ${popularClass} reveal" style="animation-delay:${i * 0.08}s">
        ...
        <!-- عدد الشدات + أيقونة UC -->
        <div class="pkg-uc-row">
          <img class="uc-coin-img" src="https://img.icons8.com/fluency/48/coin.png" alt="UC" />
          <span class="pkg-uc">${pkg.uc.toLocaleString()}</span>
          <span class="pkg-uc-tag">UC</span>
        </div>

        <!-- خانة اختيار الكمية -->
        <div class="qty-box">
          <button class="qty-btn" onclick="changeQty('qty-${pkg.id}', 'price-uc-${pkg.id}', -1, ${pkg.price_egp})">−</button>
          <input class="qty-input" id="qty-${pkg.id}" type="number" value="1" min="1" max="99"
            oninput="updatePrice('qty-${pkg.id}', 'price-uc-${pkg.id}', ${pkg.price_egp})" />
          <button class="qty-btn" onclick="changeQty('qty-${pkg.id}', 'price-uc-${pkg.id}', 1, ${pkg.price_egp})">+</button>
        </div>

        <div class="pkg-price-box">
          <span class="pkg-price" id="price-uc-${pkg.id}" data-base="${pkg.price_egp}">
            ${pkg.price_egp} جنيه
          </span>
        </div>
        <button class="btn-buy" onclick="buyWithQty('${pkg.uc} UC', ${pkg.price_egp}, 'qty-${pkg.id}')">
          🛒 اشتري الآن
        </button>
      </div>
    `;
...
// تغيير الكمية بزرار + أو -
function changeQty(inputId, priceId, delta, basePrice) {
  const input = document.getElementById(inputId);
  if (!input) return;
  let val = parseInt(input.value) || 1;
  val = Math.max(1, Math.min(99, val + delta));
  input.value = val;
  updatePrice(inputId, priceId, basePrice);
}

// تحديث السعر بناءً على الكمية
function updatePrice(inputId, priceId, basePrice) {
  const input = document.getElementById(inputId);
  const priceEl = document.getElementById(priceId);
  if (!input || !priceEl) return;
  const qty = Math.max(1, parseInt(input.value) || 1);
  const total = qty * basePrice;
  priceEl.textContent = CONFIG.currency === "USD"
    ? `$${(total / CONFIG.currencyRate).toFixed(2)}`
    : `${total.toLocaleString()} جنيه`;
}

// شراء مع مراعاة الكمية
function buyWithQty(packageName, priceEGP, qtyInputId) {
  const input = document.getElementById(qtyInputId);
  const qty = input ? Math.max(1, parseInt(input.value) || 1) : 1;
  const total = qty * priceEGP;
  const priceDisplay = CONFIG.currency === "USD"
    ? `$${(total / CONFIG.currencyRate).toFixed(2)}`
    : `${total.toLocaleString()} جنيه مصري`;

  const message = encodeURIComponent(
    `مرحباً، أريد شراء:\n` +
    `📦 الباقة: ${packageName}\n` +
    `🔢 الكمية: ${qty}\n` +
    `💰 الإجمالي: ${priceDisplay}\n\n` +
